...
class _MasarAppState extends State<MasarApp> {
  ...
  void initState() {
    ...
    if (!kIsWeb && !Platform.environment.containsKey('FLUTTER_TEST')) {
      _intentDataStreamSubscription = (ReceiveSharingIntent as dynamic).getMediaStream().listen((dynamic value) {
      ...
      (ReceiveSharingIntent as dynamic).getInitialMedia().then((dynamic value) {
        if (value is List && value.isNotEmpty) {
          _saveSharedLink(value.first.path);
        }
      });
