...
class _MasarAppState extends State<MasarApp> {
  ...
  void initState() {
    ...
    if (!kIsWeb && !Platform.environment.containsKey('FLUTTER_TEST')) {
      _intentDataStreamSubscription = (ReceiveSharingIntent as dynamic).instance.getTextStream().listen((String value) {
        _saveSharedLink(value);
      }, onError: (err) {
        print("getLinkStream error: $err");
      });

      (ReceiveSharingIntent as dynamic).instance.getInitialText().then((String? value) {
        if (value != null) {
          _saveSharedLink(value);
        }
      });
