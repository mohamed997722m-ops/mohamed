...
class _MasarAppState extends State<MasarApp> {
  ...
  void initState() {
    ...
    if (!kIsWeb && !Platform.environment.containsKey('FLUTTER_TEST')) {
      _intentDataStreamSubscription = (ReceiveSharingIntent.instance as dynamic).getMediaStream().listen((dynamic value) {
        if (value != null && (value as List).isNotEmpty) {
          _saveSharedLink(value.first.path as String);
        }
      }, onError: (err) {
        print("getMediaStream error: $err");
      });

      (ReceiveSharingIntent.instance as dynamic).getInitialMedia().then((dynamic value) {
        if (value != null && (value as List).isNotEmpty) {
          _saveSharedLink(value.first.path as String);
        }
      });
